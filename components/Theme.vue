<template>
  <v-card rounded outlined @click="router.push('/themes/' + theme.slug)" class="ma-1">
    ...
    <v-card-actions>
      <v-progress-linear :indeterminate="loading" :model-value="responseCount" :max="questionCount" min="0"
        color="primary" height="10" rounded=""></v-progress-linear>
      <div class="text-caption">{{ responseCount }}/{{ questionCount }}</div>
      <div v-if="mastery > 0" class="text-caption d-flex"><v-icon class="mr-1">mdi-arm-flex</v-icon> {{
        mastery.toFixed(1) }}</div>
    </v-card-actions>
  </v-card>
</template>
