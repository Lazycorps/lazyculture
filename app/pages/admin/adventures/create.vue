<template>
  <div class="w-full px-4 py-2 select-none">
    <div v-if="loading" class="flex flex-col items-center justify-center h-96 space-y-4">
      ...
      <p class="text-sm text-gray-500 font-medium font-display">Chargement des données...</p>
    </div>
    <AdminAdventureEditor v-else :themes="themes ?? []" @saved="onSaved" @cancelled="onCancelled" />
  </div>
</template>
