...
`shouldEndRunOnDamage(hp) = hp <= 0`. Filets de sécurité pouvant annuler une mort, dans cet ordre : relique **Seconde Chance** (`hasExtraLife`, consommée une fois) puis consommable **Dernier Souffle**/`REVIVE_TOKEN` (auto-déclenché, jamais utilisable manuellement) — les deux ramènent à 1 PV. `finalizeRun` calcule XP final, convertit l'or restant en Points de Savoir, crédite les pièces du dernier palier si `WON`, et déclenche les achievements `brainrunGames`/`brainrunWins`.
...
- Conversion or→Points de Savoir en fin de run (`WON`/`LOST`/`ABANDONED`) : `goldToKnowledgePoints`, taux `BRAINRUN_KP_PER_GOLD` = 0.2, arrondi inférieur.
- 4 talents permanents (`shared/brainrunTalents.ts`, achat unique, pas de paliers) : Constitution (+1 PV départ), Œil affûté (bonus de poids reliques RARE), Bas de laine (+20 or départ), Frappe assurée (+3 dégâts boss/bonne réponse, s'additionne à la relique Adrénaline). Appliqués via `getActiveTalentEffects`, même mécanisme d'agrégation que les reliques.
- Déblocage protégé par un débit atomique conditionnel (`unlockTalent` dans `brainrunMetaHelper.ts`), même filet que l'achat en Boutique.
