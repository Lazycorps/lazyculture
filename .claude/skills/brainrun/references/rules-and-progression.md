...
3. `acknowledgeRoom()` — confirme le récap de fin de salle (`CLEARED`) et avance vers la rangée suivante via `advanceAfterRoomClear`. Bloqué tant qu'une **carte de thème** (post-combat gagné), un bonus post-combat ou un choix de thème banni est en attente. Séquence après un combat gagné : récap → carte de thème → (bonus relique/consommable pour Elite/Boss) → salle suivante ; le Boss final ne déclenche ni carte ni bonus. Détail → `theme-coefficients.md`.
...
- Difficulté **par type de combat** (`BRAINRUN_DIFFICULTY_BY_COMBAT_TYPE`, plage inclusive 1-5, **à plat sur les actes** depuis le rework des coefficients de thème) : STANDARD [1,3] / ELITE [1,4] / BOSS [2,5]. `culture_generale` garde sa propre plage par acte (`BRAINRUN_CULTURE_GENERALE_DIFFICULTY_BY_ACT` : [1,2]/[2,3]/**[3,5]**) car ce thème est quasi systématique dans les pools ennemis/boss — évite qu'il soit toujours trop facile ou toujours difficile. Avant d'ajouter un thème/une plage, vérifier le volume réel de questions (voir `enemies-and-bosses.md`).
- **Coefficients de thème** : le joueur oriente quels thèmes tombent en combat en investissant via des cartes post-combat ; le tirage des questions est pondéré par thème (coef joueur + bonus de l'ennemi). Système complet, cartes et anti-répétition → `theme-coefficients.md`.
