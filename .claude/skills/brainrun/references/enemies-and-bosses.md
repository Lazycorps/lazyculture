...
**⚠️ Piège du gel d'affichage pendant le feedback (bug corrigé le 2026-09-10)** — corollaire direct du mécanisme ci-dessus, et le plus vicieux du malus : quand l'Antidote laisse le tampon d'avance retomber à 1, l'état renvoyé par `submitAnswer` lui-même est **déjà** en phase de mémorisation (`currentQuestion` null + `previewQuestion` non null), alors que le joueur est encore en train de lire le feedback de la réponse qu'il vient de valider. `BrainrunQuestionRunner.vue` dérivait `isAlainIntroPhase` directement de `brainrun.isAlainMemoryIntro` (état serveur brut, non gelé, contrairement à `localQuestion`/`localPreviewQuestion`) : l'écran basculait donc instantanément sur le décompte « Retenez bien cette question ! », ce qui démontait le `<template v-else>` — donc `QuestionDisplay` **et** la barre d'action portant le bouton « Continuer », seul déclencheur de `prepareNextBossQuestion`. Résultat : plus de question à valider, plus de bouton, `questionDeadline` encore `null` (posé à `null` par `submitAnswer`) donc décompte figé et timer arrêté — combat bloqué définitivement, PV et or de la run perdus. Correctif : `isAlainIntroPhase` est un `computed` **gelé derrière `localQuestion`** (`brainrun.isAlainMemoryIntro && !localQuestion`), comme tout le reste de l'affichage du composant — le feedback et son bouton « Continuer » restent affichés, et la phase de mémorisation n'apparaît qu'après le clic, une fois `localQuestion` remis à `null` par `nextQuestion()`. **Règle générale à retenir** : dans ce composant, aucune bascule d'affichage ne doit être dérivée directement de l'état serveur — `submitAnswer` renvoie l'état de l'étape _suivante_ en un seul aller-retour, il faut donc systématiquement passer par les snapshots locaux.

Un **filet de sécurité** complète le correctif (`startAlainIntroCountdown` + son `watch`) : si l'écran arrive en phase de mémorisation avec `questionDeadline` à `null` sans être passé par « Continuer » (rechargement de page, combat repris plus tard, run laissée bloquée par le bug ci-dessus avant correctif), il appelle `readyNextBossQuestion()` pour démarrer le décompte. Il est volontairement restreint aux questions suivantes (`responses.length > 0`) : le décompte de la toute première est démarré par la transition d'entrée en combat (`handleCombatIntroDone`/`handleEnteringCombatDone` dans `app/pages/brainrun/index.vue`), et l'anticiper grignoterait le temps de mémorisation pendant que l'overlay d'entrée masque encore la question.
