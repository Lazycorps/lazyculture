...
## Phase 5 — Arbre de talents ✅ Terminée (migration `20260711120000_brainrun_talent_tree`, 2026-07-11)

- [x] Les 4 talents plats de la Phase 4 remplacés par un arbre à 3 branches (Résistance/Dégâts/Utilitaire, 7 nœuds chacune, prérequis explicites, coûts par palier) — détail complet dans `.claude/skills/brainrun/references/talents.md`
- [x] Système de Bouclier reconstruit en charges (`BrainrunRun.shieldCharges`, remplace le booléen `shieldArmed`) partagées entre le consommable Bouclier et 2 nouveaux talents, expirant en fin de combat
- [x] Ordre des filets de résurrection inversé (consommable → relique → talent, au lieu de relique → consommable) et étendu d'un 3e palier (talent ultime Second Souffle, 2 PV, 1×/run)
- [x] Nouveaux mécanismes de combat de boss : plancher de dégâts, réduction de PV max au démarrage, suppression du timeout sur la 1re réponse d'un combat
- [x] Nouvelle UI d'arbre (`BrainrunTalentTree.vue`, 3 instances dans `talents.vue`) reprenant la technique d'arêtes SVG de `BrainrunMap.vue`
- [x] Script one-off de remboursement des joueurs ayant déjà débloqué un ancien talent (`scripts/brainrun-talent-tree-refund.mjs`, déjà exécuté)
- [x] `vp check` : 0 erreur. `vp test brainrunLogic` : 120/120 tests passent. `vue-tsc --noEmit` : aucune erreur nouvelle (5 erreurs pré-existantes hors-scope)
